...
export const VideoPlaylistView: React.FC<Props> = ({ 
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      ...
      if (data && data.videoPlaylist && Array.isArray(data.videoPlaylist)) {
          ...
          // Legacy support
          setPlaylist([
              { title: 'Lecture 1', url: data.premiumVideoLink || data.freeVideoLink || '', price: data.price || 5 }
          ]);
      } else {
          setPlaylist([]);
      }
  ...
  const handleVideoClick = (index: number) => {
      ...
      const price = video.price || 5; // Default cost if not set
